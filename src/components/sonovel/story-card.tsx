...
export function StoryCard({ series, onClick, favorited, listenPercent }: { series: SeriesItem; onClick?: () => void; favorited?: boolean; listenPercent?: number }) {
  ...
  return (
    ...
    >
      <div className="relative aspect-[3/4] w-full overflow-hidden rounded-lg border border-border bg-muted shadow-sm transition-all duration-200 group-hover:shadow-lg group-hover:border-primary/40">
        ...
        <div className="absolute top-1.5 left-1.5 right-1.5 flex items-start justify-between gap-1">
          <div className="flex flex-col gap-1">
            {series.status === 'completed' && (
              <Badge variant="secondary" className="bg-background/85 backdrop-blur text-[10px] px-1.5 py-0 shadow-sm">Hoàn thành</Badge>
            )}
            {series.status === 'published' && (
              <Badge variant="secondary" className="bg-emerald-500/90 text-white backdrop-blur text-[10px] px-1.5 py-0 shadow-sm">Đang ra</Badge>
            )}
            {series.status === 'draft' && (
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-1.5 pt-4">
          <div className="flex items-center gap-2 text-[10px] text-white/95">
            <span className="flex items-center gap-0.5"><BookOpen className="h-3 w-3" /> {series.chapterCount ?? 0} ch</span>
      ...
      <div className="mt-1.5 px-0.5">
        ...
        {primaryGenre && (
          <span className="mt-1 inline-block text-[10px] text-primary/80 font-medium">{primaryGenre}</span>
        )}
      </div>
    </button>
  )
