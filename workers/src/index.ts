...
app.get('/api/series/:id/illustrations', async (c) => {
  ...
  const result = await cachedFetch(`illust:${id}`, 60_000, async () => {
    const rows = await c.env.DB.prepare('SELECT id, order_no, image_url, thumb_url, group_name, caption FROM series_illustrations WHERE series_id=? ORDER BY order_no ASC').bind(id).all<any>()
    return { items: (rows.results ?? []).map((r) => ({ id: r.id, imageUrl: r.image_url, thumbUrl: r.thumb_url || r.image_url, groupName: r.group_name || '', caption: r.caption || '', orderNo: r.order_no })) }
  })
  return c.json(result)
})
...
app.put('/api/series/:id/illustrations', async (c) => {
  ...
  for (const it of items) {
    ...
    if (!url) continue
    stmts.push(
      db.prepare('INSERT INTO series_illustrations (id, series_id, order_no, image_url, thumb_url, group_name, caption, created_at) VALUES (?,?,?,?,?,?,?,?)')
        .bind(uuid(), id, count, url.slice(0, 2000), String(it?.thumbUrl || '').slice(0, 2000), String(it?.groupName || '').slice(0, 200), String(it?.caption || '').slice(0, 500), now)
    )
    count++
  }
