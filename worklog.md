...
- [x] Phase 3: Migrate Supabase → D1 — 8 series + 2161 chương + 15 tags (24.55 MB snapshot, 23.73 MB SQL, 2189 queries, D1 34.48 MB), script `scripts/migrate-supabase-to-d1.ts:1` (fetch Supabase service_role + chunk 200 + esc + wrangler execute --file), verify `wrangler d1 execute COUNT(*)` 8/2161/15 + Worker `GET /api/tags 200 15` + `GET /api/series?limit=2 200 total 8` + `GET /api/series/:id 360 chương`, redeploy `9a07f421…` clear cache
- [x] Phase 4: Web proxy — `src/lib/worker.ts:1` (`getWorkerHeaders` Bearer/service + `proxyToWorker`), 30 files `src/app/api/**` proxy sang `WORKER_URL` (catalog `series`/`chapters`/`tags`, user `progress`/`favorites`/`history`/`bookmarks`/`settings`/`continue-listening`/`stats/*`, admin `series`/`chapters/bulk`/`tags`/`admin/users` hybrid + `import-folder` FS+bulk), `eslint` pass, push `d52aea6` → `main`
- [x] Phase 5: Mobile → Worker — `mobile/src/lib/worker.ts:1` (`WORKER_URL` hardcode + `Authorization Bearer`), `mobile/src/lib/progress.ts:1` (catalog `listSeries`/`getSeries`/`listChapters` → Worker `GET /api/series*`, user `progress`/`favorites`/`history` dual write Worker+Supabase + fallback), `mobile/src/lib/chapters.ts:1` (`GET /api/chapters/:id`), `mobile/src/lib/tts.ts:188` (`flushTtsSave` dual `PUT /api/progress`), `npx tsc --noEmit` pass
